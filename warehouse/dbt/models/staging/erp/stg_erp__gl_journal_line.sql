...
joined as (
    select
        ...
        case
            when extract(month from h.posting_date) >= 2
            then extract(year from h.posting_date)
            else extract(year from h.posting_date) - 1
        end                                                 as fiscal_year,

        case
            when extract(month from h.posting_date) in (2, 3, 4)  then 1
            when extract(month from h.posting_date) in (5, 6, 7)  then 2
            when extract(month from h.posting_date) in (8, 9, 10) then 3
            else 4
